...
class TrackballHolderCreator:
    ...
    def _create_semi_shpere(self) -> Part:
        outer_sphere = Sphere(radius=self.SPHERE_INNER_RADIUS + self.THICKNESS)
    ...
    def _add_bearings(self, result: Part) -> Part:
        ...
        outer_cyl = loc1 * Cylinder(radius=self.BEARING_RADIUS + self.THICKNESS, 
    ...
    def _create_foot_box(self) -> Part:
        ...
        foot_dy = self.THICKNESS/2 - self.BOTTOM_PLATE_Y_LEN/2
    ...
    def _create_foot_connections(self) -> Part:
        foot_height = self.SENSOR_BRACKET_HEIGHT + 2 * self.SLOT_LEN
        foot_conn = Pos(Z=foot_height/2) * Box(self.BOTTOM_PLATE_X_LEN, self.THICKNESS, foot_height)

        slot_width = self.THICKNESS + 2 * TOLERANCE
        slot_box = Pos(Z=self.SLOT_LEN/2) * Box(slot_width, 100.0, self.SLOT_LEN)

        x_off = (self.SLOT_DIST - slot_width) / 2
